...
| **Plugin beta release** Chauvet `3.29.43`+ for Manta / Nomad, `2.26.40`+ for A5 X / A6 X | **[v2.0.11](../../releases/tag/v2.0.11)** | Rebuilt for `sn-plugin-lib` 0.1.65: declares & requests the file permissions the new firmware requires. Adds the dot-grid templates and the Dutch language, fixes the title zone on notes made for another device, and fixes template install on devices running several plugins (the templates are now written straight to MyStyle, without reading the plugin's private dir). |
...
## Screen flashing — fixed on Chauvet `3.29.43` / `2.26.40` and later

The two firmware bugs this plugin used to work around are **fixed by Ratta on
Chauvet `3.29.43` / `2.26.40` and later**, so the plugin runs
...
On **Chauvet `3.29.42` / `2.26.39`** both bugs are still
present: the page
...
The `main` branch targets **Chauvet `3.29.43` / `2.26.40`** (`sn-plugin-lib`
0.1.65, file-permission handling). To build the **Chauvet `3.29.42` /
`2.26.39`** version instead, check out the `v1.0.0` tag first.

Porting your own plugin to Chauvet `3.29.43` / `2.26.40`? Field notes here:
